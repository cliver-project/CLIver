...
def test_add_stdio_mcp_server(load_cliver, init_config):
    ...
    # update it
    result = CliRunner().invoke(load_cliver, ["config", "mcp", "set", "--name", "blender", "--command", "npx"])
    assert result.exit_code == 0
    ...
    # remove it
    result = CliRunner().invoke(load_cliver, ["config", "mcp", "remove", "--name", "blender"])
    assert result.exit_code == 0
...
def test_add_llm_simple(load_cliver, init_config):
    ...
    result = CliRunner().invoke(load_cliver, ["config", "llm", "add",
                                              "--name", "deepseek",
                                              "--name-in-provider", "deepseek-coder:6.7b",
                                              "--provider", "ollama",
    ...
    # update it
    result = CliRunner().invoke(load_cliver, ["config", "llm", "set",
                                              "--name", "deepseek",
                                              "--name-in-provider", "deepseek-coder:6.7b",
                                              "--provider", "vllm"])
    assert result.exit_code == 0
    ...
    # remove it
    result = CliRunner().invoke(load_cliver, ["config", "llm", "remove",
                                              "--name", "deepseek"])
    assert result.exit_code == 0
