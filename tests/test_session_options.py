...
def test_session_options_individual_updates(load_cliver, init_config, simple_llm_model):
    ...
    # Test model update
    result = runner.invoke(load_cliver, ["session", "--model", "test-model"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Set model to 'test-model' for this session" in result.output

    # Test temperature update
    result = runner.invoke(load_cliver, ["session", "--temperature", "0.8"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Set temperature to 0.8" in result.output

    # Test max-tokens update
    result = runner.invoke(load_cliver, ["session", "--max-tokens", "1024"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Set max_tokens to 1024" in result.output

    # Test top-p update
    result = runner.invoke(load_cliver, ["session", "--top-p", "0.9"], obj=cliver_instance)
    assert result.exit_code == 0
    ...
    # Test stream enable
    result = runner.invoke(load_cliver, ["session", "--stream"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Enabled streaming" in result.output

    # Test no-stream (to disable)
    result = runner.invoke(load_cliver, ["session", "--no-stream"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Disabled streaming" in result.output

    # Test save-media enable
    result = runner.invoke(load_cliver, ["session", "--save-media"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Enabled save-media" in result.output

    # Test no-save-media (to disable)
    result = runner.invoke(load_cliver, ["session", "--no-save-media"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Disabled save-media" in result.output

    # Test media-dir update
    result = runner.invoke(load_cliver, ["session", "--media-dir", "/tmp/test"], obj=cliver_instance)
    assert result.exit_code == 0
    assert "Set media_dir to '/tmp/test'" in result.output

    # Test included-tools update
    result = runner.invoke(load_cliver, ["session", "--included-tools", "all"], obj=cliver_instance)
    assert result.exit_code == 0
