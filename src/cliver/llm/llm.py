...
class AgentCore:
    ...
    async def _maybe_learn_skill(self, user_input: str, result: BaseMessage, ctx: "CallContext") -> None:
    ...
    async def stream_user_input(
        ...
        if ctx.generated_media:
            yield AIMessageChunk(
                content="",
                additional_kwargs={"media_content": ctx.generated_media},
            )
    ...
    async def process_user_input(
        ...
        if ctx.generated_media and isinstance(result, AIMessage):
            kwargs = dict(result.additional_kwargs) if result.additional_kwargs else {}
            kwargs["media_content"] = ctx.generated_media
        ...
        await self._maybe_learn_skill(user_input, result, ctx=ctx)
            ...
            while iteration < max_iterations:
                ...
                if tool_calls:
                    stop, result = await self._execute_tool_calls(
                        ...
                        tool_error_check,
                        ctx,
                        llm_response=accumulated_chunks,
                    )
                    if stop:
                        if result:
                            # noinspection PyArgumentList
